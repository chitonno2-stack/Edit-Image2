        ...
        />
         {!isGemini && (
            <div className="text-xs bg-yellow-800/30 border border-yellow-500 text-yellow-300 p-2 rounded-lg">
              Chế độ OpenAI/DALL-E sẽ tạo ra một ảnh chân dung mới dựa trên mô tả, không chỉnh sửa ảnh gốc.
            </div>
          )}
      </PanelSection>

      <div className={`p-2 transition-opacity ${!isGemini ? 'opacity-50 cursor-not-allowed' : ''}`}>
        <button 
          onClick={handleInstantRemaster}
          disabled={!isGemini}
          className="w-full font-semibold bg-gradient-to-r from-green-500 to-green-700 hover:from-green-600 hover:to-green-800 rounded-lg py-3 transition-all text-white shadow-lg disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed"
        >
